...
static std::shared_ptr<Instruction> make_inst(const int position, const std::string& line) {
    ...
    for (int p = 0; p < position; p++)
        linhas.push_back("add $t0, $t0, $t0"); // dummy: apenas ocupa a posição
...
int main() {
    ...
    secao("1.3 InstructionFactory — arquitetura de trace (MIPS32)");
    {
    ...
    secao("[ABORT] Registrador inválido deve abortar");
    {
    ...
    */
    ...
    secao("6.2 BRANCH — normalização de labels e registradores mistos");
    {
    ...
    secao("7.1 Aliases apontam para o mesmo registrador físico ($8 == $t0, $0 == $zero)");
    {
    ...
    secao("7.2 '$' é obrigatório para virar registrador — imediatos numéricos nunca colidem");
    {
        auto i = make_inst(1, "addi $t1, $t1, 100");
        check("imediato '100' (sem '$') não vira fonte", i->GetSourceRegisters().size() == 1);
    }

    secao("7.3 lui — só dest, nenhuma fonte (imediato de 16 bits, não é '$')");
    {
    ...
    secao("7.4 Grupos de aliases vizinhos não colidem ($t8/$t9 vs $k0/$k1)");
    {
    ...
    secao("7.5 Label parecido com nome de registrador (sem '$') NÃO é confundido");
    {
